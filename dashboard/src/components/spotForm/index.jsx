...
export default function SpotForm() {
  ...
  const { scenesState, handleCloseDrawerSpot, currentSceneStateOpen, temporaryMarker, openDrawerSpot, setOpenDrawerSpot, setCurrentSceneStateOpen, setScenesState, setTemporaryMarker, groupSceneId, groupScene, setGroupScene } = useAppStateContext()
  ...
  const onSubmitSpot = async(data) => {
    setIsLoading(true)
      let payload = {
        ...
        position_y: temporaryMarker.y,
        position_z: temporaryMarker.z
      }

      try{
        const createdSpot = await createSpotFromScene(groupSceneId,currentSceneStateOpen.id, payload);
        ...
        if(data.poiFile) {
          ...
          payload.media.path = uploadResponse.data.path
          payload.media.type = uploadResponse.data.type
        }
        ...
        if(groupScene && groupScene.data){
          ...
          const newArraySpots = [...selectedScene.spots, payload]
      ...
      >
        <div className="flex flex-col gap-8 h-full">
          ...
          <form onSubmit={poiForm.handleSubmit(onSubmitSpot)} className="h-full flex flex-col items-center justify-between gap-8">
            ...
            <div className="flex flex-col gap-4 w-full">
              <button type="submit" disabled={isLoading} className="bg-black text-white py-4 rounded-full text-xl flex items-center justify-center">
                {
                  isLoading ? (
