...
const Markers: React.FC<MarkersProps> = ({ spots, temporaryMarker, onMarkerClick }) => {
  ...
  const getTextureByType = (spotType: string) => {
    if(spotType.includes('image')){
      return spotImage;
    }else if(spotType.includes('video')){
      return spotVideo;
    }else {
      return spotText;
    }
        ...
        const position = new Vector3(position_x, position_y, position_z);
          return (
            ...
            >
              <spriteMaterial 
              attach="material" 
              map={getTextureByType(media?.type ?? 'text/html')} 
              depthTest={false}
                depthWrite={false}
...
export const SceneEdit: React.FC<props> = () => {
