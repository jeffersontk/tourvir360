...
export function SceneItemEdit({scene, index}:props) {
  const { handleDeleteSceneList, handleOpenScene, deleteSceneLoading} = useAppStateContext()
  return (
    <div 
      className="flex flex-col w-full h-52 md:h-52 lg:h-52 mb-4"
    >
      <div className="relative w-full h-full">
      {scene && scene.media && <Image
        ...
        <div className="absolute inset-0 flex justify-center items-center bg-black bg-opacity-50">
          <Tooltip className="bg-white text-blackNissan" content="Adicionar ponto de interesse">
            <button onClick={()=> handleOpenScene(scene)} className="bg-white rounded-full p-4 shadow-lg hover:bg-gray-100" aria-label="Adicionar ponto de interesse">
